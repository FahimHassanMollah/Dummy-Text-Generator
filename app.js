...
document.querySelector('#generate-button').addEventListener('click',function (e) {
  ...
  let ammountStringValue=document.getElementById('amount').value;
  let ammount=Number.parseInt(ammountStringValue);
  if (ammount<=0||ammount>9||isNaN(ammount)){
    result.innerHTML=`<p>${dummyText[random]}</p>`
  }
  else {
    let sliceArray=dummyText.slice(0,ammount);
    let finalString='';
    sliceArray.map(function (value) {
